...
def wdl_to_label(info: dict) -> float | None:
    """Extract a White-perspective expected-score label from Stockfish's
    WDL output, matching the W/D/L base corpus shape exactly.

    With `UCI_ShowWDL = true`, Stockfish emits a `wdl <win> <draw> <loss>`
    triplet per info line. python-chess wraps this as
    `info["wdl"]: PovWdl`; pivoting to White perspective and squashing
    to an expected-score in [0, 1] gives `(wins + draws/2) / 1000`. This
    is the natural calibrated probability that the same SF would score
    a unit point as White from this position, which is exactly the
    shape the W/D/L base corpus carries (averaged game outcomes from
    self-play). Returns None when the engine did not emit WDL (older
    SF that does not support UCI_ShowWDL, or some other UCI engine);
    callers fall back to `cp_to_label` in that case.
    """
    wdl = info.get("wdl")
    if wdl is None:
        return None
    w = wdl.white()
    return (w.wins + w.draws / 2.0) / 1000.0
...
def evaluate_position(engine: chess.engine.SimpleEngine, fen: str,
                      depth: int, time_cap_s: float):
    """Run Stockfish on a position; return (label, cp).

    `label` is from Stockfish's `wdl` info field (calibrated win/draw/loss
    probabilities in per-mille) converted to a White-perspective
    expected-score in [0, 1] via `wdl_to_label`. Falls back to None when
    the engine did not emit WDL; callers convert via `cp_to_label` in
    that case. `cp` is always the White-perspective centipawn score
    clamped to +/-1500, kept for logging and for the cp fallback path.
    ...
    cp = score_to_white_cp(info["score"])
    label = wdl_to_label(info)
    return label, cp
...
def main():
    ...
    try:
        ...
        for i, (fen, tag) in enumerate(positions):
            try:
                label, cp = evaluate_position(engine, fen, args.depth, args.time_cap_s)
                ...
                continue
            if label is None:
                # SF too old to support UCI_ShowWDL, or some other UCI
                # engine that does not emit wdl info. Fall back to the
                # cp-sigmoid path so the row still has a label.
    ...
    with open(out_path, "w") as f:
        ...
        label_source = "wdl" if wdl_misses == 0 else f"wdl ({wdl_misses} cp-fallback)"
        f.write(f"# stockfish-depth={args.depth} label-source={label_source}\n")
    ...
    if wdl_misses:
        print(f"note: {wdl_misses} rows fell back to cp-sigmoid "
              f"(SF did not emit wdl)", file=sys.stderr)
