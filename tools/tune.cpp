...
static std::vector<ParamRef> collectParams() {
    ...
    // ReachableOutpost is a smaller positive cousin of KnightOutpostBonus;
    // BadOutpost is the layered penalty for the wing-outpost shape that has
    // nothing concrete to attack. Sign-locked so neither can drift through
    // zero and silently turn into the other term.
    addMgEgConstr("ReachableOutpost", &evalParams.ReachableOutpost, boundsNonNegative());
    addMgEgConstr("BadOutpost", &evalParams.BadOutpost, boundsNonPositive());
    ...
    // Bishop x-ray family: pure bonuses on both halves. BishopOnKingRingXRay
    // is mg-only because the eg default is structurally zero (king safety
    // matters in the middlegame). RookOnQueenFile is the slider-meets-queen
    // file bonus, also a pure positive feature. QueenInfiltration carries
    // mixed signs by construction (mg negative, eg positive), so it stays
    // unconstrained the way the addMgEg default already permits.
    addMgEgConstr("BishopXRayPawns", &evalParams.BishopXRayPawns, boundsNonNegative());
    out.push_back(
        {"BishopOnKingRingXRay.mg", &evalParams.BishopOnKingRingXRay, true, boundsNonNegative()});
    ...
    // same pattern Tempo and the storm terms already follow. Mg keeps
    // the chain `[0] >= [1] >= 0` so the back-rank shield is always at
    // least as valuable as the rank-3 shield.
    ...
    // KingSafeCheck[pt][1] is the multi-attacker dimension and tunes
    // mg-only with a non-negative floor (see classical-aligned block
    // below).
    for (bool isMg : {true, false}) {
        out.push_back({"KingSafeCheck[2][0]." + std::string(isMg ? "mg" : "eg"),
                       &evalParams.KingSafeCheck[2][0], isMg, [isMg] {
                           Bounds b{0, 1000000};
                           int rook = isMg ? mg_value(evalParams.KingSafeCheck[4][0])
                                           : eg_value(evalParams.KingSafeCheck[4][0]);
                           b.hi = std::min(b.hi, rook);
                           return b;
                       }});
        out.push_back({"KingSafeCheck[3][0]." + std::string(isMg ? "mg" : "eg"),
                       &evalParams.KingSafeCheck[3][0], isMg, [isMg] {
                           Bounds b{0, 1000000};
                           int rook = isMg ? mg_value(evalParams.KingSafeCheck[4][0])
                                           : eg_value(evalParams.KingSafeCheck[4][0]);
                           b.hi = std::min(b.hi, rook);
        ...
        out.push_back({"KingSafeCheck[4][0]." + std::string(isMg ? "mg" : "eg"),
                       &evalParams.KingSafeCheck[4][0], isMg, [isMg] {
                           Bounds b{0, 1000000};
                           int knight = isMg ? mg_value(evalParams.KingSafeCheck[2][0])
                                             : eg_value(evalParams.KingSafeCheck[2][0]);
                           int bishop = isMg ? mg_value(evalParams.KingSafeCheck[3][0])
                                             : eg_value(evalParams.KingSafeCheck[3][0]);
                           int queen = isMg ? mg_value(evalParams.KingSafeCheck[5][0])
                                            : eg_value(evalParams.KingSafeCheck[5][0]);
                           b.lo = std::max({b.lo, knight, bishop});
                           b.hi = std::min(b.hi, queen);
                           return b;
                       }});
        out.push_back({"KingSafeCheck[5][0]." + std::string(isMg ? "mg" : "eg"),
                       &evalParams.KingSafeCheck[5][0], isMg, [isMg] {
                           Bounds b{0, 1000000};
                           int rook = isMg ? mg_value(evalParams.KingSafeCheck[4][0])
                                           : eg_value(evalParams.KingSafeCheck[4][0]);
    ...
    out.push_back(
        {"KingBlockerWeight.mg", &evalParams.KingBlockerWeight, true, boundsNonNegative()});
    ...
    // WeakLever: an unsupported pawn under two enemy pawn attackers is a
    // sure material loss; the term must stay a penalty.
    addMgEgConstr("WeakLever", &evalParams.WeakLever, boundsNonPositive());
    ...
    out.push_back({"InitiativeAlmostUnwinnable.eg", &evalParams.InitiativeAlmostUnwinnable, false,
                   boundsNonPositive()});
    out.push_back(
        {"KingPawnDistance.eg", &evalParams.KingPawnDistance, false, boundsNonPositive()});
    addMgEgConstr("PassedFile", &evalParams.PassedFile, boundsNonPositive());
    ...
    // --- Weak-queen-protection (mg-only structurally; eg=0). Tying the
...
static void printCurrentValues() {
    ...
    std::cout << "    " << fmtScore(evalParams.ReachableOutpost) << ", // ReachableOutpost\n";
    std::cout << "    " << fmtScore(evalParams.BadOutpost) << ", // BadOutpost\n";
    ...
    std::cout << "    " << fmtScore(evalParams.BishopOnKingRingXRay)
              << ", // BishopOnKingRingXRay\n";
    ...
    std::cout << "    {\n";
    for (int pt = 0; pt < 7; pt++) {
        std::cout << "        {" << fmtScore(evalParams.KingSafeCheck[pt][0]) << ", "
    ...
    std::cout << "    " << fmtScore(evalParams.KingBlockerWeight) << ", // KingBlockerWeight\n";
    std::cout << "    " << fmtScore(evalParams.KingKnightDefenderDiscount)
              << ", // KingKnightDefenderDiscount\n";
    std::cout << "    " << fmtScore(evalParams.PawnlessFlank) << ", // PawnlessFlank\n";
    std::cout << "    " << fmtScore(evalParams.KingFlankAttack) << ", // KingFlankAttack\n";
    ...
    std::cout << "    " << fmtScore(evalParams.WeakLever) << ", // WeakLever\n";
