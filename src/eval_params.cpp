...
static const EvalParams kDefaultEvalParams = {
    ...
    S(60, 0), // KnightOnQueen
    S(-282, -106), // PawnlessFlank
    S(0, 8), // QueenInfiltration
    S(0, 0), // KingPawnDistEg
    S(0, 40), // KBNKCornerEg
    S(0, 250), // LucenaEg
    // HEAD-only fields below this line: neutral defaults so the new
    // structural code (endgame conversion gradients, drawishness scales,
    // SEE-aware threat damping) contributes as little as possible to the
    // overall eval. Push/close fields set to 0 (no extra gradient).
    // Drawishness scales pinned to their SPSA-allowed minimums.
    // EscapableThreatScale pinned to its SPSA-allowed maximum so escapable
    // threats get the most credit available (37.5%, vs main's effective
    // 100%; this is the closest we can get without loosening the bound).
    S(0, 0), // KXKPushToEdge
    S(0, 0), // KXKPushClose
    S(0, 0), // KBNKPushClose
    ...
    S(0, 16), // KRKPDrawishScale  (SPSA min = 16)
    S(0, 16), // KRKMinorScale     (SPSA min = 16)
    S(0, 0), // KNNKDrawScale
    S(0, 24), // EscapableThreatScale (SPSA max = 24)
};
