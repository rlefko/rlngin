...
TEST_CASE("Tunable registry: exposes the expected SPSA surface", "[tunable]") {
    ...
    // Fifty seven is the committed SPSA surface: twenty original scalars,
    ...
    // piece-on-king-ring scalars, two pawn-islands penalty scalars, the
    // qsearch delta margin, the late-history pruning coefficient, and
    // the four attack-aware ordering and threat-based LMR scalars. If
    // this count ever changes, the SPSA driver's iteration budget and
    // the PR description should move too.
    REQUIRE(specs.size() == 57);
