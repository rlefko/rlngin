...
TEST_CASE("Tunable registry: exposes the expected SPSA surface", "[tunable]") {
    ...
    // Seventy five is the committed SPSA surface: twenty original scalars,
    ...
    // and threat-based LMR scalars, and ten new HCE refactor scalars
    // (BadBishop, BishopPawns, BishopXrayPawns, RookOnQueenFile,
    // KingMobilityFactor, each as Mg/Eg pair). If this count ever
    // changes, the SPSA driver's iteration budget and the PR description
    // should move too.
    REQUIRE(specs.size() == 75);
